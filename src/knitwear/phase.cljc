(ns knitwear.phase
  "Phase table for Knitwear Manufacturing Plant Operations state graph.
...
(def phase-table
  "State graph topology for knitwear-manufacturing plant operations coordination.
  Entry: ADVISOR-NODE
  Output nodes: COMPLETE-NODE (approved), HOLD-NODE (blocked)

  Flow:
    advisor -> proposes operation (batch logging, maintenance, safety
               concern, shipment)
    governor -> evaluates against hard/soft gates
    if holds (hard violation): HOLD (escalate to human, cannot override)
    if clean or only soft violations: COMPLETE (record approved by human)
  "
  {:start ADVISOR-NODE
   :nodes {ADVISOR-NODE {:type :function
                         :description "LLM advisor proposes knitwear plant operations"}
